...
$ans4i=$_POST['ans4i'];
$status=$_POST['sts'];
$stmt = $conn->prepare("update questions set q_text=?,q_text_image=?,q_ans1=?,q_ans1_image=?,q_ans2=?,q_ans2_image=?,q_ans3=?,q_ans3_image=?,q_ans4=?,q_ans4_image=?,q_correct=?,q_active=? where q_id=?");
$stmt->bind_param("ssssssssssiii", $qstn,$qstni,$ans1,$ans1i,$ans2,$ans2i,$ans3,$ans3i,$ans4,$ans4i,$cans,$status,$qid);
	if($stmt->execute())
	{
		mysqli_commit($conn);
        echo "<b style='color:green'>Updated successfully</b><br/>";		
	} 
	else 
	{
		echo "<b style='color:red'>Update Failed</b><br/>";
	}
$y="select * from questions where q_active!=2 and regulation_ids=$b and dept_ids='$c' and years=$years and sems=$sems and subject_codes='$d' order by q_id DESC";
getTheory($y);	
}
